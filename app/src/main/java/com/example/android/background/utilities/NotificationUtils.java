...
public class NotificationUtils {
    ...
    public static void remindUserBecauseCharging(Context context) {
        ...
        Builder notificationBuilder = new Builder(context,WATER_REMINDER_NOTIFICATION_CHANNEL_ID)
    ...
    public static Action ignoreReminderAction(Context context){
        Intent intentDismissWaterReminderService = new Intent(context, WaterReminderIntentService.class);
        intentDismissWaterReminderService.setAction(ReminderTasks.ACTION_DISMISS_NOTIFICATION);

        PendingIntent waterReminderPendingIntent = PendingIntent.getService(
                context,
                ACTION_IGNORE_PENDING_INTENT_ID,
                intentDismissWaterReminderService,
                PendingIntent.FLAG_UPDATE_CURRENT);

        Action ignoreReminderAction = new Action(
                R.drawable.ic_cancel_black_24px,
                "No, thanks",
                waterReminderPendingIntent);
    ...
    public static Action drinkWaterAction(Context context){
        Intent intentWaterReminderService = new Intent(context, WaterReminderIntentService.class);
        intentWaterReminderService.setAction(ReminderTasks.ACTION_INCREMENT_WATER_COUNT);

        PendingIntent intentWaterReminderPendingIntent = PendingIntent.getService(
                context,
                ACTION_DRINK_PENDING_INTENT_ID,
                intentWaterReminderService,
                PendingIntent.FLAG_UPDATE_CURRENT);

        Action action = new Action(
                R.drawable.ic_local_drink_black_24px,
                "I did it!",
                intentWaterReminderPendingIntent);
